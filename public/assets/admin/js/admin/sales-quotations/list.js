/**
 * Sales quotation list DataTable.
 */
$(function () {
    var table = $('#masterTable').DataTable({
        ...
        columns: [
            ...
            { data: 'customer', orderable: false }, { data: 'status', orderable: false },
            { data: 'grand_total', orderable: false },
            { data: 'action', orderable: false, searchable: false }
        ]
    });
    $('#filterStatus, #filterCustomer').on('change', function () { table.ajax.reload(null, false); });
    $(document).on('click', '.btn-delete-master', function () {
        var url = $(this).data('url');
        Swal.fire({ title: 'Delete draft quotation?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' }).then(function (result) {
