/**
 * Packing unit listing DataTable.
 */
$(function () {
    var table = $('#masterTable').DataTable({
        ...
        columns: [
            { data: 'id' }, { data: 'code' }, { data: 'name' }, { data: 'item', orderable: false },
            { data: 'quantity' }, { data: 'base_quantity', orderable: false },
            { data: 'is_active' }, { data: 'action', orderable: false, searchable: false }
        ]
    });

    $(document).on('click', '.btn-delete-master', function () {
        var url = $(this).data('url');
        Swal.fire({ title: 'Delete this packing unit?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' }).then(function (result) {
